...
function ComicVideoPanel({ isAdmin }: { isAdmin: boolean }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setShowModal(false); setInputUrl('')
  ...
  return (
    <>
      {/*
        Panel manga: nempel di kanan layar, sisi kiri diagonal
        Bentuk trapezoid — kiri miring, kanan flush ke tepi
        position: absolute, right: 0, overflow terpotong parent
      */}
      <div style={{
        position: 'absolute',
        top: 0,
        right: -16,          // sedikit bleeding ke kanan
        width: 152,
        height: 160,
        zIndex: 10,
        pointerEvents: 'none',
        ...
        <div style={{
          ...
          background: 'linear-gradient(135deg, #c8ff00 0%, #7fff50 50%, #c8ff00 100%)',
        }} />

        {/* Video layer - 2px inset dari border */}
        <div style={{
          position: 'absolute',
          ...
          {/* Halftone pojok kiri bawah (di sisi diagonal) */}
          <div style={{
            position: 'absolute', bottom: 8, left: 8, width: 40, height: 40,
            backgroundImage: 'radial-gradient(circle, rgba(200,255,0,0.45) 1.2px, transparent 1.2px)',
        ...
        {/* Corner tick — pojok kanan atas */}
        <div style={{ position:'absolute', top:0, right:0, width:22, height:22, zIndex:5, pointerEvents:'none' }}>
          <svg width="22" height="22" viewBox="0 0 22 22">
            <polyline points="8,2 20,2 20,14" fill="none" stroke="#c8ff00" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </div>
        {/* Corner tick — pojok kanan bawah */}
        <div style={{ position:'absolute', bottom:0, right:0, width:22, height:22, zIndex:5, pointerEvents:'none' }}>
          <svg width="22" height="22" viewBox="0 0 22 22">
            <polyline points="20,8 20,20 8,20" fill="none" stroke="#c8ff00" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </div>
        ...
        {isAdmin && (
          <button
            ...
            style={{
              ...
              zIndex: 6, whiteSpace: 'nowrap', pointerEvents: 'auto',
            }}
          >✎ GANTI</button>
        )}
      </div>
        ...
        >
          ...
          >
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              <div style={{ border:'1px dashed rgba(200,255,0,0.3)', borderRadius:8, padding:'12px', textAlign:'center', position:'relative', background:'#0d1200' }}>
                ...
                <input type="file" accept="video/*" disabled={saving}
                  onChange={async (e) => {
                    ...
                    try { const url = await uploadToCloudinary(file); setInputUrl(url); setUploadProgress(null) }
                ...
                {uploadProgress !== null ? (
                  <div style={{ display:'flex', flexDirection:'column', gap:6, alignItems:'center' }}>
                    <div style={{ width:'100%', height:4, background:'#1a2200', borderRadius:4, overflow:'hidden' }}>
                      <div style={{ height:'100%', width:`${uploadProgress}%`, background:'#c8ff00', borderRadius:4, transition:'width 0.2s' }} />
                    </div>
                    <span style={{ fontFamily:'monospace', fontSize:9, color:'#c8ff00' }}>UPLOADING {uploadProgress}%</span>
                  </div>
                ) : inputUrl.includes('cloudinary') ? (
                  <span style={{ fontFamily:'monospace', fontSize:9, color:'#c8ff00' }}>✓ Berhasil!</span>
                ) : (
              ...
              <input type="text" placeholder="URL Video..." value={inputUrl} onChange={e => { setInputUrl(e.target.value); setErr('') }} disabled={saving}
              ...
              <div style={{ display:'flex', gap:8, marginTop:4 }}>
                <button onClick={handleSave} disabled={saving || !inputUrl.trim()}
                  style={{ flex:1, padding:'12px', borderRadius:10, background: saving?'#2a3a00':'linear-gradient(135deg,#c8ff00,#a0cc00)', color: saving?'#c8ff00':'#000', fontWeight:800, fontSize:12, border:'none', cursor: saving?'default':'pointer', fontFamily:'monospace' }}>
                  {saving ? 'PROSES...' : 'SIMPAN'}
                </button>
                <button onClick={() => { if (!saving) { setShowModal(false); setErr(''); setUploadProgress(null) } }}
                  style={{ padding:'12px 16px', borderRadius:10, background:'#1a1a1a', border:'1px solid #333', color:'#888', fontSize:12, cursor:'pointer' }}>
                  Batal
                </button>
              </div>
            </div>
...
function App() {
  ...
  return (
    ...
    <div className="page">
      ...
      <main>
        <section className="hero section" id="beranda" style={{ overflow: 'hidden' }}>
