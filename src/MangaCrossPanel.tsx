...
// ═══════════════════════════════════════════════════════
// CLOUDINARY CONFIG
// Tambah akun baru di sini kalau mau multi-akun nanti
// ═══════════════════════════════════════════════════════
const CLOUDINARY_ACCOUNTS = [
  { name: 'kyoko1', label: 'Cloudinary #1 (25GB)', cloudName: 'dtxpdx8ua', uploadPreset: 'kyokoapp' },
  // { name: 'kyoko2', label: 'Cloudinary #2 (25GB)', cloudName: 'xxxxx', uploadPreset: 'kyokoapp' },
]
...
export default function MangaCrossPanel({ isAdmin, userId }: Props) {
  ...
  const [uploadCloudinary, setUploadCloudinary] = useState(CLOUDINARY_ACCOUNTS[0].name)
  ...
  useEffect(() => {
    const unsubs: (() => void)[] = []
    // Pisah per-firebase biar kalau ada yang dihapus, bisa di-remove dari map
    const mangasByFb: Record<string, Record<string, MangaDoc>> = {}

    FB_INSTANCES.forEach(fb => {
      mangasByFb[fb.name] = {}
      const q = query(collection(fb.db, 'mangaCross'), orderBy('createdAt', 'desc'))
      const unsub = onSnapshot(q, snap => {
        // Reset map untuk firebase ini, lalu isi ulang dari snapshot terbaru
        // Ini otomatis handle delete: doc yang dihapus tidak akan ada di snap.docs
        const fresh: Record<string, MangaDoc> = {}
        snap.docs.forEach(d => {
          const data = d.data()
          fresh[d.id] = {
            id: d.id,
        ...
        mangasByFb[fb.name] = fresh

        // Gabung semua firebase, sort by createdAt
        const all = Object.values(mangasByFb).flatMap(m => Object.values(m))
        setMangas(all.sort((a, b) => b.createdAt - a.createdAt))
      })
      unsubs.push(unsub)
    })
  ...
  const uploadCover = async (mangaId: string, fb: FbInstance): Promise<string> => {
    if (coverInputMode === 'url' && uploadCoverUrl.trim()) {
      return uploadCoverUrl.trim()
    }
  ...
  const uploadToCloudinary = async (blob: Blob, filename: string, cloudName: string, uploadPreset: string): Promise<string> => {
    ...
    if (!res.ok) throw new Error(`Cloudinary upload gagal: ${res.statusText}`)
    const data = await res.json()
    return data.secure_url as string
  }

  // ── Extract ZIP and upload pages ke Cloudinary (urut, satu per satu) ──
  const uploadZipPages = async (mangaId: string, chapterId: string): Promise<{ urls: string[], bytes: number }> => {
    ...
    const cloudAcc = CLOUDINARY_ACCOUNTS.find(c => c.name === uploadCloudinary) || CLOUDINARY_ACCOUNTS[0]
    let totalBytes = 0
    const urls: string[] = []
    ...
    for (let i = 0; i < imageFiles.length; i++) {
      ...
      const filename = `${mangaId}_${chapterId}_${String(i + 1).padStart(3, '0')}.${ext}`
      setUploadStatus2(`Upload halaman ${i + 1}/${imageFiles.length}...`)
      setUploadProgress(40 + ((i / imageFiles.length) * 55))
      const url = await uploadToCloudinary(blob, filename, cloudAcc.cloudName, cloudAcc.uploadPreset)
      urls.push(url)
  ...
  const handleUpload = async () => {
    if (!uploadTitle.trim() || (!uploadCoverFile && !uploadCoverUrl.trim()) || !uploadZipFile) {
    ...
    try {
      ...
      // 4. Upload ZIP pages ke Cloudinary
      setUploadStatus2('Mengekstrak & upload halaman ZIP ke Cloudinary...')
      const { urls: pages, bytes: chapterBytes } = await uploadZipPages(mangaId, chapterRef.id)
  ...
  const resetUpload = () => {
    ...
    setUploadCoverUrl(''); setCoverInputMode('file')
      ...
      <div style={S.wrap}>
        ...
        <div style={{ overflowY: 'auto', flex: 1, padding: 14 }}>
          ...
          <div style={S.formGroup}>
            <label style={S.label}>🔥 Pilih Firebase (untuk data manga)</label>
          ...
          {/* Pilih Cloudinary */}
          <div style={S.formGroup}>
            <label style={S.label}>☁️ Pilih Cloudinary (untuk gambar chapter)</label>
            <select style={S.select} value={uploadCloudinary} onChange={e => setUploadCloudinary(e.target.value)}>
              {CLOUDINARY_ACCOUNTS.map(c => (
                <option key={c.name} value={c.name}>{c.label}</option>
              ))}
            </select>
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', marginTop: 4 }}>
              ☁️ Gambar chapter disimpan di Cloudinary, bukan Firebase Storage
            </div>
          </div>
            ...
            <>
              ...
              <div style={S.formGroup}>
                ...
                <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                  {(['file', 'url'] as const).map(mode => (
                    <button key={mode} style={{
                      ...
                      background: coverInputMode === mode ? 'rgba(200,245,0,0.15)' : 'rgba(255,255,255,0.05)',
                      border: `1px solid ${coverInputMode === mode ? 'rgba(200,245,0,0.5)' : 'rgba(255,255,255,0.1)'}`,
                      color: coverInputMode === mode ? '#c8f500' : 'rgba(255,255,255,0.5)',
                    }} onClick={() => { setCoverInputMode(mode); setUploadCoverFile(null); setUploadCoverPreview(''); setUploadCoverUrl('') }}>
                      {mode === 'file' ? '📁 Upload File' : '🔗 Pakai URL'}
                    </button>
                  ))}
                </div>
                ...
                {coverInputMode === 'file' ? (
                  ...
                  <>
                    <input style={S.input} placeholder="https://files.catbox.moe/xxx.jpg atau raw GitHub..."
                      value={uploadCoverUrl}
                      onChange={e => { setUploadCoverUrl(e.target.value); setUploadCoverPreview(e.target.value) }} />
                    <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', marginTop: 4 }}>
                      💡 Bisa pakai: Catbox, GitHub raw, imgbb, dll. Pastikan URL langsung ke gambar.
                    </div>
                    {uploadCoverUrl && (
                      <img src={uploadCoverUrl} alt="preview"
                        onError={e => (e.currentTarget.style.display = 'none')}
                        onLoad={e => (e.currentTarget.style.display = 'block')}
                        style={{ width: 100, height: 145, objectFit: 'cover', borderRadius: 6, marginTop: 8, display: 'none' }} />
                    )}
                  </>
                )}
              </div>
